				...
				<ul class="ul">
					...
					<li>
				     	...
				     	<my-pop class="ccpop" v-model="i.flag">
				     		<div class="pop_item">
				     			<span>电影名</span>
				     			<h2 style="font-size: 24px;">《{{ruleFrom.moviesName}}》</h2>
				     		</div>
				     		<div class="pop_item">
				     			<span>简介</span>
      ...
      methods:{
	     ...
	     beforeAvatarUpload(file) {
            ...
            if(this.imageUrl||this.ruleFrom.pic){
		  	  	//console.log(this.ruleFrom.pic)
		  	  	let res = await this.$http.put("/movie?id="+i._id,this.ruleFrom);
//		  	  	console.log(this.tableData)
		  	  	this.$message({
		  	  		message:res.data,
		  	  		type:'success'
	  	  ...
	  	  async handleDelete(i){
	  	  	...
	  	  	let res = await this.$http.delete("/movie?id="+i._id);
	  	  	this.$message({
	  	  		message:res.data,
	  	  		type:'success'
	  	  	})
