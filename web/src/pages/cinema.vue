<template>
	<div class="cinema">
		<my-header></my-header>
		<header>
			<router-link :to="{name:'all',params:{aa:'movies'}}" tag="div" class="el-icon-finished">片单</router-link>
			<router-link :to="{name:'files',params:{aa:'movies'}}" tag="div" class="el-icon-files">找电影</router-link>
			<router-link :to="{name:'data',params:{aa:'movies'}}" tag="div" class="el-icon-s-data">榜单</router-link>
		</header>
		<main>
			<div class="main_one">
				<div class="h2">
					<h2>影院热映</h2>
					<router-link tag="h2" :to="{name:'all',params:{aa:'movies'}}"><span>全部 {{ruleFrom.length}}</span><i class="el-icon-arrow-right"></i></router-link>
				</div>
				<!--影院热映-->
				<div class="content">
					<div class="content_item" v-for="(i,j) in tableFrom" :key="j">
						<el-button class="el-icon-plus" :plain="true" @click="tarch(i)"></el-button>
						<img :src="i.pic" @click="GOdetail(i)"/>
					...
					<div class="row">
						<div class="col">
							...
							<div class="image_text">
								<ul>
									<li v-for="(i,j) in tableA" :key="j" @click="GOdetail(i)">{{j+1}}. {{i.moviesName}}</li>
								</ul>
							</div>
						</div>
						<div class="col">
							...
							<div class="image_text">
								<ul>
									<li v-for="(i,j) in tableB" :key="j" @click="GOdetail(i)">{{j+1}}. {{i.moviesName}}</li>
								</ul>
							</div>
						</div>
					</div>
			...
			<div class="main_three">
				...
				<div class="btn_group">
					<div>
						<router-link :to="{name:'all',params:{aa:0}}" tag='button' class="btn_near">近期</router-link>
						<router-link :to="{name:'all',params:{aa:0,index:'中国'}}" tag='button' class="btn_classics">中国</router-link>
						<router-link :to="{name:'all',params:{aa:0,index:'美国'}}" tag='button' class="btn_small">美国</router-link>
						<router-link :to="{name:'all',params:{aa:0}}" tag='button' class="btn_play"><i class="el-icon-video-play"></i> 可播放</router-link>
					</div>
					<router-link :to="{name:'files',params:{aa:0}}" tag='span' class="btn_near el-icon-thumb">筛选</router-link>
				</div>
				<div class="content">
...
export default {
    ...
    methods:{
    	...
    	async tarch(data){
  	 	...
  	 	change(current){
	  	    ...
	  	    //console.log(this.total,this.current*this.size,(this.total-this.size))
				...
				this.current++;
				this.getMoviesCurrentData(current)
			}else{
	    ...
	    async getMoviesCurrentData(current){
	  	  	let res = await this.$http.get("/moviescurrent",{
...
.el-notification {background: rgba(0,0,0,.8);margin-bottom: 1.5rem;width: 5rem;}
.el-notification__content{color: #fff;font-size: 0.28rem;}
.el-button{border-radius: 4px;}
.el-tabs__header {margin-top: 0.3rem;}
.el-rate__icon{
